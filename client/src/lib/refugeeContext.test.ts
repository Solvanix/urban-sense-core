...
describe("refugee context route", () => {
  ...
  it("states that the route does not collect beneficiaries, donations, or aid requests", () => {
    expect(refugeeContextGuards.join(" ")).toContain("لا توجد هنا استمارة مساعدة");
    expect(refugeeContextGuards.join(" ")).toContain("لا توجد تبرعات");
    expect(refugeeContextGuards.join(" ")).toContain("لا تمثيل للأونروا");
  });
});
