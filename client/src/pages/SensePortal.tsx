...
import { Accessibility, ArrowLeft, Bot, Building2, ChevronLeft, CircleDollarSign, ClipboardList, Database, Eye, Gauge, Layers3, LockKeyhole, Map, MessageCircle, Radio, Route, ShieldCheck, Store, UsersRound, Wrench } from "lucide-react";
...
export const ecosystemRoutes: EcosystemRoute[] = [
  { id: "urban-sense", label: "Urban‑Sense", eyebrow: "البلاغات البلدية", readiness: "منشور", description: "منصة بلاغات بلدية عربية: تقديم، مراجعة، إسناد، أدلة، تحقق وإغلاق.", detail: "له قاعدة بيانات وهوية وصلاحيات خاصة به. لا يحتوي سياحة أو متجرًا.", actionLabel: "افتح المنصة البلدية", href: "/", internal: true, icon: Building2, tone: "teal" },
  ...
  return <main dir="rtl" className={cn("min-h-screen bg-[#091419] text-[#f7f8f1] selection:bg-[#e3a238] selection:text-[#152225]", highContrast && "bg-black text-white")} data-reduced-motion={reducedMotion ? "true" : "false"}>
    <div className="mx-auto max-w-7xl px-5 pb-16 pt-5 lg:px-8">
      <header className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-5">
        <div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#e3a238] text-[#142729]"><Layers3 size={22} /></span><div><p className="text-lg font-extrabold tracking-tight">SENSE</p><p className="text-xs font-bold text-white/55">بوابة المسارات — نسخة توجيهية</p></div></div>
      ...
      <section className="grid gap-8 py-10 lg:grid-cols-[.83fr_1.17fr] lg:items-end lg:py-16">
        <div><p className="inline-flex items-center gap-2 rounded-full border border-[#e3a238]/35 bg-[#e3a238]/10 px-3 py-1.5 text-xs font-extrabold text-[#ffd881]"><Radio size={14} />لوحة توجيه لا حساب موحّد</p><h1 className="mt-5 max-w-xl text-4xl font-extrabold leading-[1.17] tracking-tight sm:text-5xl">كل مسار له <span className="text-[#e3a238]">محطته</span> وحدوده.</h1><p className="mt-5 max-w-2xl text-lg leading-8 text-white/65">هذه ليست صفحة تدّعي أن كل شيء جاهز أو موصول في قاعدة واحدة. إنها مدخل واحد يوضح أين يبدأ كل منتج، وما الذي يعمل الآن، وما الذي ينتظر قرارًا أو تشغيلًا مستقلًا.</p><div className="mt-8 flex flex-wrap gap-3 text-sm font-bold text-white/70"><span className="inline-flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-emerald-300" />منشور</span><span className="inline-flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-sky-300" />نواة مستقلة</span><span className="inline-flex items-center gap-2"><span className="h-2.5 w-2.5 rounded-full bg-amber-300" />مقترح</span></div></div>
