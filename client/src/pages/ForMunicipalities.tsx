      ...
      <section className="mx-auto max-w-7xl px-5 pb-14 pt-8 lg:px-8 lg:pb-20 lg:pt-14">
        <div className="grid gap-8 rounded-[2rem] border border-teal-900/10 bg-white p-7 shadow-xl shadow-teal-950/5 lg:grid-cols-[1.05fr_.95fr] lg:p-12">
          <div>
            ...
            <div className="mt-8 flex flex-wrap gap-3"><Link href="/التجربة" className="inline-flex h-12 items-center gap-2 rounded-xl bg-[#0f5b5b] px-6 font-bold text-white transition-colors hover:bg-[#0a4848]">فهم نطاق التجربة <ArrowLeft size={18} /></Link><Link href="/كيف-تعمل" className="inline-flex h-12 items-center rounded-xl border border-slate-300 px-6 font-bold text-slate-700 transition-colors hover:bg-slate-50">عرض مسار البلاغ</Link></div>
          </div>
          <div className="relative overflow-hidden rounded-3xl bg-[#0f5b5b] p-6 text-white"><div className="absolute -left-10 -top-10 h-44 w-44 rounded-full bg-[#e3a238]/20 blur-2xl" /><div className="relative"><p className="font-extrabold text-[#f6c76d]">صورة تشغيلية مبسطة</p><div className="mt-5 space-y-3">{["المواطن: تقديم ومتابعة", "موظف الخدمة: مراجعة وتصنيف", "الفريق الميداني: تنفيذ وأدلة", "المشرف: تحقق وإغلاق"].map((item, index) => <div key={item} className="flex items-center gap-4 rounded-2xl border border-white/10 bg-white/5 p-4"><span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-white/10 text-sm font-extrabold">{index + 1}</span><p className="font-bold">{item}</p></div>)}</div></div></div>
        </div>
      </section>
