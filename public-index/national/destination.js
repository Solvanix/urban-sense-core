...
function render(item) {
  ...
  const trustRows = [['المصدر', item.sourceStatus], ['المراجعة', item.reviewStatus], ['الوصول', item.accessStatus], ['الحجز', item.bookingStatus], ['آخر تحديث', item.lastReviewed]];
  const trustTable = trustRows.map(([label, value]) => `<div class="trust-row"><b>${esc(label)}</b><span>${esc(value)}</span></div>`).join('');
  target.innerHTML = `<div class="detail-hero ${item.status === 'pilot' ? 'live' : ''}"><span class="status ${item.status !== 'pilot' ? 'candidate' : ''}">${esc(item.statusLabel)}</span><p class="destination-type">${esc(item.cluster)} · ${esc(item.region)} · ${esc(item.type)}</p><h1>${esc(item.name)}</h1><p>${esc(item.summary)}</p></div><div class="detail-grid"><section class="detail-card"><p class="kicker">01 / TRUST CARD</p><h2>ما الذي نعرفه الآن؟</h2><p>هذه بطاقة تشغيلية إرشادية، وليست اعتمادًا حكوميًا أو وعدًا بالحجز. كلما نقصت البيانات، يظهر ذلك صراحة للزائر.</p><div class="trust-table">${trustTable}</div><div class="facts">${facts}</div><p class="source-line"><b>مالك المحتوى:</b> ${esc(item.ownerStatus)}</p></section><section class="detail-card"><p class="kicker">02 / READINESS</p><h2>بوابة الجاهزية</h2><ul class="readiness">${checks}</ul><p class="source-line"><b>النتيجة الإرشادية:</b> ${score}/5 عناصر مكتملة</p></section></div><div class="detail-actions"><a class="button" href="./">← العودة إلى الفهرس</a>${item.href ? `<a class="button primary" href="${item.href}">افتح المسارات المنشورة ↗</a>` : ''}</div>`;
}
