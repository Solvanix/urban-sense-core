...
describe("SENSE ecosystem portal route inventory", () => {
  it("labels the only public application as published and keeps the other routes honest", () => {
    ...
    expect(ecosystemRoutes.filter((route) => route.readiness === "مقترح").map((route) => route.id)).toEqual(["commerce", "maker", "masaken"]);
  });
  ...
  it("keeps Masaken Al-Khair as a criteria board rather than a housing listing or payment flow", () => {
    const masaken = ecosystemRoutes.find((route) => route.id === "masaken");
    expect(masaken).toMatchObject({ readiness: "مقترح", internal: true, href: "/?view=masaken" });
    expect(masaken?.detail).toContain("ليست قوائم سكن");
  });
