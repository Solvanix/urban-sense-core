...
const lanes = [
  {
    ...
    icon: CheckCircle2,
    items: [
      { title: "Urban‑Sense", text: "منصة بلاغات بلدية عربية تشمل التقديم والمتابعة ضمن حدود البلدية والدور.", href: liveUrl, action: "افتح المنصة" },
      { title: "بوابة SENSE والمستكشف", text: "مدخل عام يوضح مستويات المنظومة وروابطها وحالاتها بدل إخفاء الفجوات.", href: `${liveUrl}${sensePortalHref}`, action: "افتح البوابة" },
    ],
  },
  {
  ...
  },
  {
    ...
    icon: LockKeyhole,
    items: [
      { title: "الدومين والبريد والكيان", text: "لا يوجد اسم أم أو نطاق أو بريد مهني معتمد بعد؛ قرار الاسم يسبق الشراء وDNS والترحيل.", href: domainReadinessHref, action: "افتح الجاهزية" },
      { title: "السوق والتجارة", text: "يتطلب كتالوجًا حقيقيًا، مزودين معتمدين، مدفوعات وسياسات إرجاع قبل أن يصبح تجربة عامة.", href: null, action: "يتطلب قرار التجارة" },
    ],
  },
];
    ...
    <main dir="rtl" className="min-h-screen bg-[#071216] text-[#f7f8f1]">
      <div className="mx-auto max-w-7xl px-5 pb-16 pt-5 lg:px-8">
        <header className="flex flex-wrap items-center justify-between gap-4 border-b border-white/10 pb-5">
          <Link href={ecosystemExplorerHref} className="inline-flex items-center gap-2 text-sm font-extrabold text-[#f7cc71]"><ArrowLeft size={16} />العودة إلى خريطة الوصول</Link>
          <Link href={sensePortalHref} className="inline-flex min-h-10 items-center gap-2 rounded-xl border border-white/15 px-3 text-sm font-extrabold text-white/80">بوابة SENSE <Compass size={16} /></Link>
        </header>

        <section className="grid gap-8 py-12 lg:grid-cols-[1.1fr_.9fr] lg:items-end">
          <div>
            <p className="inline-flex items-center gap-2 rounded-full border border-[#e3a238]/35 bg-[#e3a238]/10 px-3 py-1.5 text-xs font-extrabold text-[#ffd881]"><Route size={14} />لوحة تقدم تشغيلية</p>
            <h1 className="mt-5 max-w-4xl text-4xl font-extrabold leading-[1.14] sm:text-6xl">لا نقيس التقدم بوعود كبيرة، بل بما <span className="text-[#e3a238]">يمكن زيارته ومسؤوليته واضحة.</span></h1>
            <p className="mt-5 max-w-3xl text-lg leading-8 text-white/65">هذه ليست نسبة إنجاز مصطنعة. إنها خريطة عمل: ما نُشر، وما بُني لكنه مقيد، وما يحتاج قرارًا أو تشغيلًا حقيقيًا قبل أن يصبح خدمة.</p>
          </div>
        ...
        <section className="grid gap-5 xl:grid-cols-3" aria-label="حالات تشغيل المنظومة">
          {lanes.map((lane) => {
            ...
            return <article key={lane.title} className={`flex min-h-[440px] flex-col rounded-[2rem] border p-6 ${lane.tone}`}>
              <div className="flex items-start justify-between gap-4"><div><p className={`text-sm font-extrabold ${lane.labelTone}`}>{lane.title}</p><p className="mt-3 text-sm leading-7 text-white/60">{lane.summary}</p></div><span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white/10"><Icon size={22} /></span></div>
              <div className="mt-7 space-y-4">{lane.items.map((item) => <div key={item.title} className="rounded-[1.35rem] border border-white/10 bg-[#071216]/45 p-5"><h3 className="text-lg font-extrabold">{item.title}</h3><p className="mt-2 text-sm leading-6 text-white/60">{item.text}</p>{item.href ? <a href={item.href} target={item.href.startsWith("http") ? "_blank" : undefined} rel={item.href.startsWith("http") ? "noreferrer" : undefined} className="mt-5 inline-flex items-center gap-2 text-sm font-extrabold text-[#f7cc71]">{item.action}<ArrowLeft size={15} /></a> : <span className="mt-5 inline-flex items-center gap-2 text-sm font-extrabold text-white/45">{item.action}<Store size={15} /></span>}</div>)}</div>
            </article>;
          })}
        </section>
